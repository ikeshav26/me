...
const Form = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/api/messages/create`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="w-full mt-17 mb-8">
      <div
        className={`max-w-2xl mx-auto rounded-xl p-1 ${theme === 'dark' ? 'bg-white/3 border-white/5' : 'bg-black/3 border-black/10'} border`}
      >
        <div
          className={`${theme === 'dark' ? 'bg-[#080808]' : 'bg-white'} rounded-xl p-6 sm:p-8 transition-colors duration-300`}
        >
          <div className="mb-8">
            <h2
              className={`text-xl font-semibold ${theme === 'dark' ? 'text-white' : 'text-black'} mb-2`}
            >
              Send me a message
            </h2>
            <p
              className={`${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'} text-sm`}
            >
              ...
              <span className="relative z-10 flex items-center gap-2">
                {isSubmitting ? (
                  <>
                    <Loader2
                      size={16}
                      className={`animate-spin ${theme === 'dark' ? 'text-black' : 'text-white'}`}
                    />
                    Sending...
                  </>
                ) : (
                  <>
                    ...
                    <ArrowUpRight
                      size={16}
                      className={`${theme === 'dark' ? 'text-gray-500 group-hover:text-black' : 'text-gray-400 group-hover:text-white'} group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300`}
                    />
                  </>
                )}
              </span>
