...
const Guestbook = () => {
  ...
  const fetchEntries = async () => {
    try {
      ...
      setError(
        'Could not load guestbook. Please make sure the backend is running.'
      );
    } finally {
      setLoading(false);
    }
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      if (!payload.userId) {
        throw new Error(
          'Authentication data is incomplete. Please try logging out and in again.'
        );
      }
  ...
  return (
    <div className="min-h-screen pt-18 pb-20 px-4 md:px-0 max-w-2xl mx-auto">
      ...
      >
        <h1
          className={`oswald-font text-5xl md:text-6xl tracking-tight ${theme === 'dark' ? 'text-white' : 'text-black'} m-0 uppercase mb-4`}
        >
          GUESTBOOK
        </h1>
        <p
          className={`text-base md:text-lg font-['JetBrains_Mono'] max-w-xl font-light leading-relaxed ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}
        >
          Leave a feedback, share your thoughts, or just say hi. Your words will
          stay here forever.
        </p>
      </motion.div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <Avatar
                  src={user.avatarUrl}
                  alt={user.name}
                  className="w-10 h-10 rounded-full border-2 border-orange-300/50"
                />
                <div>
                  <div
      ...
      <div className="mt-34">
        <div className="flex items-center gap-3 mb-8">
          ...
          <h2
            className={`oswald-font text-2xl uppercase tracking-wider ${theme === 'dark' ? 'text-white' : 'text-black'}`}
          >
            RECENT SIGNATURES
          </h2>
          <div
            className={`flex-1 h-px ${theme === 'dark' ? 'bg-white/10' : 'bg-black/10'}`}
          ></div>
        </div>
