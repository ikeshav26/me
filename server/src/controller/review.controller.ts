  ...
  try {
    ...
    if (!userId || !reviewText) {
      return res
        .status(400)
        .json({
          message: 'Please try again later... no identification provided',
        });
    }
    ...
    if (!findReviewer) {
      return res
        .status(404)
        .json({ message: 'Please try again later... no reviewer found' });
    }
...
export const deleteReview = async (req: Request, res: Response) => {
  try {
    ...
    const findReviewer = await Reviewer.findById(userId);
    if (!findReviewer) {
